...
import { Save, Send, CheckCircle, AlertTriangle, ExternalLink, Eye, EyeOff, Lock, Loader2, Plus, X, Pencil, Trash2, GripVertical, Users } from 'lucide-react';
import { sendTeamsMessage } from '../../services/teams';
import { sendLineMessage } from '../../services/line';
import { getAppConfig, saveAppConfig, getMembers, addMember, updateMember, deleteMember } from '../../services/index';
...
export default function SettingsPage() {
  ...
  async function handleMemberSubmit(ev: React.FormEvent) {
    ev.preventDefault();
    if (editingMember?.id) {
  ...
  async function handleMemberDelete(m: Member) {
      ...
      <section className="bg-white rounded-xl border border-gray-100 p-5 mb-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-gray-800">部員管理</h2>
              <p className="text-xs text-gray-500">部員の追加・編集・削除・並び替え</p>
            </div>
          </div>
          <button
            onClick={() => { setEditingMember(null); setMemberForm(EMPTY_MEMBER); setShowMemberForm(true); }}
            className="flex items-center gap-1 bg-green-600 text-white px-3 py-2 rounded-lg text-sm hover:bg-green-700 shrink-0"
          >
            <Plus size={16} /> 部員追加
          </button>
        </div>
        <div className="space-y-2">
          ...
          {members.map((m, index) => (
            <div
              ...
              onDrop={onDrop}
              className="bg-white rounded-xl border border-gray-100 p-3 flex items-center gap-3 transition-shadow"
            >
              <div
                draggable
              ...
              <div className="flex gap-1 shrink-0">
                <button onClick={() => { setEditingMember(m); setMemberForm({ name: m.name, department: m.department, email: m.email, phone: m.phone, role: m.role }); setShowMemberForm(true); }} className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded">
                  <Pencil size={14} />
                </button>
                <button onClick={() => handleMemberDelete(m)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
